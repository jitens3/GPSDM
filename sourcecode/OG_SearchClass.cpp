...
double end;
    ...
    std::string chi2Path = "./results3/chihit" + std::to_string(week) + std::to_string(day) + ".out";
    std::ofstream chi2(chi2Path);
    
    std::string chi3Path = "./results5/t2chids" + std::to_string(week) + std::to_string(day) + ".out";
    std::ofstream test(chi3Path);
    
    std::string chi4Path = "./results5/chi1k" + std::to_string(week) + std::to_string(day) + ".out";
    std::ofstream chi3(chi4Path);

    std::string clkPath = "./results5/nclk_" + std::to_string(week) + std::to_string(day) + ".out";
    std::ofstream clkp(clkPath);
...
//  for(int j0 = jbeg; j0 <= jend; j0+=increment){
  for(int j0 = jbeg; j0 <= jend; j0++){
    ...
    for (int ix=0; ix< NMC; ix++){
      ...
      double signal[num_clocks][MAXJW];
      ...
      if(exact == 1){
        ...
        //chi_ds = calcdEs(vec, signal, JW, num_satellites);
        //chi_ss = calcChi_BLAS(m_Einv_e, *signal, *signal, num_satellites, JW);
    ...
    for (int i = MAXJW/2+1; i < num_epochs-MAXJW/2; i++){
      for(int j = 0; j < NMC; j++){
       //if(fabs(m_snr_temp[i*NMC + j]) > fabs(m_snr[i])){
              ...
              chi3 << i << " " << j << " " << tday_snr[i*NMC+j] <<" " << m_chi_temp[i*NMC + j] << "\n";
              //std::cout << "\n" << i*NMC + j;
       //     }
      }
    }
 ...
 */
 ...
 //   std::cout << "SNR: " << m_snr[100] << " " << "chi: " << m_chi[100] <<"\n";
    ...
    for (int i = MAXJW/2+1; i < num_epochs-(1 + MAXJW/2); i++){
        
        if(fabs(m_snr[i]) > 7.){
        //if(fabs(m_snr[i]) > 6.0){
            chi2 << i << "  " << m_snr[i] << " " << m_chi[i] <<" " << num_clocks << "\n";
            std::cout << "epoch: " << i << "    " << "chi[i]: " << m_chi[i] <<"	" <<"snr[i]:  " << m_snr[i]<< "\n";
            fpcount2++;
        }
...
//        }
        ...
        if(m_chi[i] > chi_thresh && fabs(m_snr[i]) > 7.0){
  //          m_snr[i] = 0.0;     // let us just set the SNR to zero, no event now.
            fpcount++;
        }
    ...
    std::cout << "SNR: " << m_snr[1200] << " " << "chi: " << m_chi[1200] <<"\n";
...
//    out_file = "./results/"+wd+"/searchData" + wd + ".out";
    out_file = "./results5/searchData" + wd + ".out";
  } 
